...
Post-contraction migrations `00063` through `00067` freeze the H3 execution and
...
The current schema is version `00067`.
...
Migrations `00063` through `00067` then bind the exact H3 execution/root-input
...
prevents assigning an unstarted graph when its Project running limit is already
exhausted; StartStage still closes races after snapshot capture. Migration
`00066` binds Stage Worker control-session takeover to strict monotonic capacity
sequence and lease-renewal semantics while making stale-session synchronization
read-only.
