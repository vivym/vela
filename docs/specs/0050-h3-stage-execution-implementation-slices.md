...
Post-contraction migrations `00063` through `00069` freeze the H3 execution and
...
exact-cache reconciliation. Migration `00066` adds durable Stage Worker
control-session takeover, read-only stale-session synchronization, strict
monotonic capacity sequence/replay authority, and renewable capacity leases.
Migration `00067` closes the first-Stage Project running-limit scheduling gap.
Migration `00068` accepts certified input Connector states before activation,
and migration `00069` makes TransferTicket and capacity expiry depend on a
post-lock server-clock sample. The current schema is version `00069`.
...
`00050_stage_graph_instantiation_dispatch.sql`, with the Admission transaction
closed by `00051_atomic_stage_graph_admission.sql`. PostgreSQL owns instantiate,
assign, first progress, completion, retry, cancellation, and bounded reconciliation;
...
v49 backfill/resume, atomic Admission rollback, deferred commit enforcement, and
empty/durable-authority migration rollback behavior. Migration `00051` closes
the prior S49.12 Admission blocker by creating the graph snapshot, initial
Attempt/StageRuns, storage reservation, command evidence, and completed work in
the same transaction as the Accepted Job and Credit Reservation. Admission also
locks and validates one complete READY capacity path across every required stage;
missing Worker/residency/latest-capacity evidence or a bounded initial READY queue returns
retryable `503 capacity_unavailable` without creating durable Job authority.
...
Migrations `00063` through `00069` then bind the exact H3 execution/root-input
...
bounded, replay-safe, and fair across repeated scans. The final migration also
revalidates TransferTicket and capacity expiry against a post-lock PostgreSQL
server-clock sample. Migration `00067` prevents assigning an unstarted graph
when its Project running limit is already exhausted; StartStage still closes
races after snapshot capture. Migration `00068` admits the exact certified input
Connector states needed by that path. Migration `00066` binds Stage Worker
control-session takeover to strict monotonic capacity sequence and lease-renewal
semantics while making stale-session synchronization read-only.
