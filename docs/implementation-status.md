...
| 0004 Organization and Project | Implemented for current API | Composite ownership and Project-scoped Admission/Get/Cancel/Artifact access plus Organization-scoped Human membership, role administration, billing, usage, audit, and settlement-contact interfaces preserve both boundaries. | Project/Organization lifecycle and policy interfaces must retain the same boundary. |
...
| 0007 Organization Isolation | Partial | Forced RLS, composite foreign keys, transaction-revalidated Human roles and sessions, separate request/auth/Human-membership/identity-administration/Organization-reporting/internal/Artifact/billing/Webhook roles, a NOLOGIN settlement-contact owner, private staging, exact-version access grants, and cross-Project/Organization administration/reporting/content negative tests. | NATS workload identity and deployment isolation evidence. |
| 0008 No Customer Content reuse | Partial | Prompt has an expiry; staging Artifacts are private; committed exact-version reads require a short-lived Project grant; Webhook payloads and Organization billing/usage/audit projections contain only safe non-content facts. | Access audit, support authorization, no-reuse policy enforcement, and deletion across storage/backups. |
...
| 0013 Non-interrupting releases | Partial | Fifteen additive migrations, exact N/N-1 database/control compatibility, an operator-receipted circuit protocol transition, Protobuf/OpenAPI breaking checks, and migration down/up evidence. | Deployed Worker/event/API rollout, drain, rollback, and retained-backlog receipts. |
