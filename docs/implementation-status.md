...
The standalone [durable assignment admission component](assignment-admission-evidence-2026-09-05.md)
adds a bounded Worker-wide watermark, original Acquire/renewal history and a
...
Production discovery retains the original transmitted Acquire command ID and
rejects assignment responses with a different or missing ID. The explicit
[durable Stream path](durable-stream-admission-evidence-2026-09-05.md) carries that
identity through Run into input/Runtime admission, renewal, Reattach and local
closure. Its CPU/race/Linux lifecycle tests pass. Default command assembly and
...
and Linux arm64 non-root process/channel tests pass. Durable retirement and the
optional in-process CPU-media adapter capability remain open.
...
supports the contract; unsupported driver drain retains the slot.
...
adds negotiated fd-4 datagrams independent of commands and read-only inspection.
The H3/thumbnail mock drivers join synchronous command work, freeze exact terminal
execution identity/sequence and preserve sealed output through reuse and shutdown.
Durable Service tests exercise successive success, STOPPED and FAILED checkpoints,
epoch recovery and resumed admission using real compiled CPU/mock commands.
...
reachable machine-level H3 Assignment path. Repository tests and the separate
...
weights, GPUs, cross-machine StageArtifact transfer, NVMe/XFS failure, long-duration soak,
