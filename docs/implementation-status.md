...
| 0004 Organization and Project | Implemented for current API | Composite ownership and Project-scoped Admission/Get/Cancel/Artifact access plus Organization-scoped Human membership and exact Project-role administration preserve both boundaries. | Project/Organization lifecycle, policy, and billing interfaces must retain the same boundary. |
...
| 0006 Fixed launch roles | Partial | All six fixed Human roles are database-constrained and auditably assignable/revocable through exact Organization/Project administration; existing Project permissions union inside one Project, while Organization roles grant no Customer Content access. | Positive BillingAdmin/OrganizationAuditor interfaces and Platform Operator Break-glass Access. |
| 0007 Organization Isolation | Partial | Forced RLS, composite foreign keys, transaction-revalidated Human roles and sessions, separate request/auth/Human-membership/identity-administration/internal/Artifact/billing/Webhook roles, private staging, exact-version access grants, and cross-Project/Organization administration/content negative tests. | NATS workload identity and deployment isolation evidence. |
...
| 0013 Non-interrupting releases | Partial | Fourteen additive migrations, exact N/N-1 database/control compatibility, an operator-receipted circuit protocol transition, Protobuf/OpenAPI breaking checks, and migration down/up evidence. | Deployed Worker/event/API rollout, drain, rollback, and retained-backlog receipts. |
...
Organization database, Human membership/role administration, Artifact, and
Webhook isolation without NATS credential evidence (27), audited Break-glass
expiry after repository-proven Human membership/role and Service Credential
mutation attribution plus non-content Human role isolation (29), and N/N-1
