...
| BusyBox Linux/amd64 Materializer Image Pinning | `6d916bb` | `docs/specs/0047-busybox-linux-amd64-image-pinning.md`, `deploy/vela-control/deployment.yaml`, `deploy/worker-agent/daemonset.yaml`, `deploy/fleet-controller/desired-revisions.yaml`, `internal/deploymentcontract/busybox_image_test.go` |
...
deployment receipt.
Slice 47 (`6d916bb`) pins the shared BusyBox `1.37.0` root materializer to its
exact `linux/amd64` OCI manifest in `vela-control`, the static Worker contract,
and Fleet desired input, then verifies all three identities through final
Kustomize renders. It does not provide registry publication, signatures, SBOM,
scan/approval evidence, a deployment, or a Launch Receipt.
