...
| Mock Runner lifecycle | Earlier restart, recovery, and all ten fixed fault rehearsals passed. Current retained terminal state on both Workers references output directories already removed by normal Worker Agent terminal cleanup, so both Runner containers restart unhealthy. The durable Job ledger remains consistent and idle | `LAB BLOCKED / PRODUCTION NOT READY`; checksum and quarantine only the two exact terminal state directories under separate approval to restore service, then fix and test the Runner/Worker terminal-state ownership contract before further experiments |
...
| `real-h3-soak` | Two Workers expose eight GPUs; success, restart, failure, cancel, one accepted durable control-plane smoke Job, and a balanced five-wave/ten-Job concurrent mock rehearsal with verified Artifacts passed before the current Runner lifecycle defect | The backend is mock, both Runner containers are currently unhealthy, the concurrent run lasted 47 seconds, and no real-H3 72-hour mixed-load or reconciliation window exists | Repair and regression-test the terminal-state lifecycle contract before any additional mock cycle; repeat the full 72-hour mixed-load contract on real H3 |
