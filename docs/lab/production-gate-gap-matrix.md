...
seven-of-nine organization-isolation rehearsal also passed 20 negative probes,
...
| `organization-isolation-content-safety` | A root-only `NON_PRODUCTION_MOCK_REHEARSAL` receipt passed `7/9` fixed scenarios and 20 negative probes: cross-Organization/project invisibility, fixed request-role matrix, private request context plus forced RLS, credential revocation, composite foreign keys, two authorized exact-version Artifact downloads, and signed URL method/path/version tampering; unexpected allows and revocation bypasses were zero | The Customer and Platform IdPs are `.invalid`; the real Break-glass approval/audit scenario and independent customer-content reuse audit were `NOT_RUN`/`NOT_MEASURED`. This partial mock receipt is not typed production evidence and cannot produce a Launch Receipt | Provision real Customer/Platform IdPs, the two-person Break-glass workflow and audit projection, and an independent content-reuse audit sink; rerun all nine scenarios with a canonical release and production identities |
