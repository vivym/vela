...
| Kubernetes/RKE2 deployment | Three-node RKE2 `v1.35.7+rke2r1` with Canal is active; API/node/CNI readiness and exact names/IPs pass. The retained `2026-08-31T02:41:14Z` strict postflight reports `FAIL failures=4`: all three live kubelet `configz` responses have `failSwapOn=false` but only `memorySwap: {}`, and 17 exact historical Failed Pods remain. API, node, DaemonSet, Deployment, Canal, and GPU `0/8/8` checks pass | `LAB RUNNING / VERIFICATION GAP / PRODUCTION NOT READY`; an explicit kubelet `NoSwap` drop-in plus RKE2 restarts requires separate approval, as does disposition of the exact historical Failed Pods. The topology also remains one shared control node, an operator-accepted stable-address assumption without DHCP reservation proof, active host swap, root-ext4 state, and no HA/quorum/DR fault domains |
