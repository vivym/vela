...
persistent mock Runner containers are also currently restarting because
terminal output cleanup removed paths required by retained terminal Runner
state. Repository tests and these lab observations cannot substitute for the
external facts required by ADR 0029.
...
| Mock Runner lifecycle | Earlier restart, recovery, and all ten fixed fault rehearsals passed. Current retained terminal state on both Workers references output directories already removed by normal Worker Agent terminal cleanup, so both Runner containers restart unhealthy. The durable Job ledger remains consistent and idle. A repository candidate now restores `SUCCEEDED` state without outputs only when the complete Attempt output directory is absent, rejects partial loss or changed content, never reexecutes, and passes all 50 Runner tests plus Worker Agent/Local Recovery tests. The aligned restart-state validator passes six complete-cleanup and corruption cases | `LAB BLOCKED / PRODUCTION NOT READY`; repository tests do not restore the live containers. Preserve the state roots, publish the fixed digest, drain and replace one exact Runner container at a time without purge, then retain terminal-replay, fresh-smoke, authority, GPU-idle, and independent postflight evidence. State quarantine is a separately approved fallback only |
