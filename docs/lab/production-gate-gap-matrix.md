...
is present. A control-only Prometheus, Alertmanager, and Grafana overlay now has
live lab evidence, but it deliberately null-routes its heartbeat and has no
external gateway SLI, paging integration, 24x7 owner, or P1 drill. Both
persistent mock Runner containers are healthy on the fixed non-canonical lab
digest after state-preserving terminal replay and fresh smoke. Repository tests
and these lab observations cannot substitute for the external facts required by
ADR 0029.
...
| Kubernetes/RKE2 deployment | Three-node RKE2 `v1.35.7+rke2r1` with Canal is active; API/node/CNI readiness and exact names/IPs pass. At `2026-08-31T04:47:12Z`, the fail-closed verifier staged at fixed SHA-256 `10b7dae8395f2d1ca1bd9fb4f8d5a73e4a98af64fb105cd24e8981f1599af24a` reported `FAIL failures=1` after the approved Worker 1, Worker 2, shared control rollout. All three raw kubelet `configz` responses now report `failSwapOn=false` and explicit `memorySwap.swapBehavior=NoSwap`; API, nodes, Canal, DaemonSets, Deployments, StatefulSets, database idle state, and GPU `0/8/8` recovered. The sole verifier failure is 16 historical Failed Pods; all owning Jobs carry `ttlSecondsAfterFinished=86400`, and no delete was issued. The root-only postflight is `/root/vela-rke2-receipts/20260831T044634Z-cluster-strict-post-noswap`, with `SHA256SUMS` SHA-256 `66ce07ecff3a29fae97eefff1666f5003bec1e87d6717a0a98696c06bf0a4e07` | `LAB RUNNING / NOSWAP VERIFIED / PRODUCTION NOT READY`; allow the existing TTL policy to expire historical Pods, then rerun the same strict verifier. Do not manually delete historical Pods as part of this rollout. The topology still has one shared control node, an operator-accepted stable-address assumption without DHCP reservation proof, active host swap, root-ext4 state, and no HA/quorum/DR fault domains; Production Gates remain `0/9 PASS` |
...
| Mock Runner lifecycle | Earlier restart, recovery, and all ten fixed fault rehearsals passed. At `2026-08-31T06:42:01Z`, both exact unhealthy containers completed an approved, state-preserving one-Worker-at-a-time rollout to non-canonical digest `sha256:33cdb7a765d35cc9b80910c5ff378e2f178e63220f423db594ffba24d5abc427`. Both cleaned terminal receipts replayed as immutable `SUCCEEDED` without outputs or backend reexecution; fresh smokes then produced two verified outputs and retired only the old terminal states. Both containers are `running/healthy`, restart count zero, validators report `checked_states=1 retired_success_states=0`, GPU compute processes are zero, and the authority boundary returned to `0|0|0|2`. State quarantine, deletion, and `--purge` were not used. Root-only image, drain/restore, rollout, replay, smoke, and independent postflight receipts are recorded in `docs/lab/three-host-mock-environment.md` | `LAB VERIFIED / PRODUCTION NOT READY`; this closes the observed terminal-cleanup restart blocker only. The image remains non-canonical and the lab still lacks real H3 backend behavior, sustained load, adjacent-release compatibility/rollback evidence, SLO qualification, production supply chain, and a Production Gate receipt |
| Control/storage fault domains | Only `marslab-server` is assigned control/storage; it is shared and had 131 GiB available memory immediately before the approved RKE2 restart | `NOT READY`; production CNPG and JetStream require three eligible independent control/storage nodes and durable disks |
...
| `real-h3-soak` | Two Workers expose eight GPUs; success, restart, failure, cancel, one accepted durable control-plane smoke Job, and a balanced five-wave/ten-Job concurrent mock rehearsal with verified Artifacts passed before the current Runner lifecycle defect. The repository fix has focused regression evidence | The backend is mock, the fix is not deployed, both Runner containers are currently unhealthy, the concurrent run lasted 47 seconds, and no real-H3 72-hour mixed-load or reconciliation window exists | Restore and roll out the lab Runner under exact receipts, rerun a mock lifecycle regression, then repeat the full 72-hour mixed-load contract on real H3 |
...
| `observability-on-call` | A control-only Prometheus `v3.14.0`, Alertmanager `v0.34.0`, and Grafana `13.2.0` overlay passed scrape, SLO exporter gauge, canonical rule, datasource/dashboard, digest, placement, zero-GPU, and Prometheus-to-Alertmanager heartbeat checks. The root-only deployment and independent postflight receipts both report `LAB_REHEARSAL_PASS`; bounded storage and rollback are documented | The external gateway SLI series count is zero; receiver `lab-null` discards the heartbeat. No external paging integration, notification acknowledgement, named and verified 24x7 owner, timed P1 drill, production HA, or production retention exists | Add the external gateway SLI source and an approved paging test target, name and verify the 24x7 owner, then run a timed P1 alert/acknowledgement/response exercise with retained evidence |
