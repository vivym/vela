...
foreign keys, and exact-version signed URL method/path/version binding. It is
explicitly `NON_PRODUCTION_MOCK_REHEARSAL`; the real IdPs, Break-glass workflow,
and content-reuse audit sink remain absent. Separately, a fresh mock Job and a
five-wave, ten-Job concurrent mock rehearsal passed the
...
| Kubernetes/RKE2 deployment | Three-node RKE2 `v1.35.7+rke2r1` with Canal is active; approved preflights, API/node/CNI readiness, and exact names/IPs passed. Fresh live kubelet `configz` reports `failSwapOn=false` and explicit `memorySwap.swapBehavior=NoSwap` on all three nodes. The `2026-08-30T23:15:27Z` strict postflight passed every check except cluster cleanliness, which found 21 retained historical failed lab Pods | `LAB RUNNING / VERIFICATION GAP / PRODUCTION NOT READY`; archive or delete only the exact historical failed Pods under separate authorization and rerun strict postflight. The topology also remains one shared control node, an operator-accepted stable-address assumption without DHCP reservation proof, active host swap, root-ext4 state, and no HA/quorum/DR fault domains |
...
| `organization-isolation-content-safety` | The reviewed v3 `NON_PRODUCTION_MOCK_REHEARSAL` receipt passed `6/9` fixed scenarios and 20 ledger-derived negative probes against two persisted synthetic foreign-scope Jobs. It binds 30 configured database URLs to exact login/group pairs; compares exact direct/effective table `22/22`, column-only `6/6`, and routine `13/13` ACLs; rejects direct login, sequence, unsafe-schema, and effective `PUBLIC`/inherited expansion; and proves forced RLS on all 72 Organization-scoped tables. Unexpected allows and credential-revocation bypasses were zero. Historical v2 bytes emitted `7/9`, but that higher value is not the current reviewed result | Request-correlated database-role telemetry, the Customer and Platform IdPs, the real Break-glass approval/audit scenario, and an independent customer-content reuse audit remain absent. The fixed-role matrix is therefore partial, and this mock receipt cannot produce a Launch Receipt | Add request-correlated role telemetry, real identities, the two-person Break-glass workflow and audit projection, and an independent content-reuse audit sink; rerun all nine scenarios with a canonical release and production identities |
