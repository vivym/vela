...
def main():
    ...
    for credential_path in [args.credential_file, args.peer_credential_file]:
    ...
    if (root / 'receipt.json').exists():
        history = root / ('receipt-before-resume-' + str(time.time_ns()) + '.json')
        history.write_bytes((root / 'receipt.json').read_bytes())
    ...
    try:
        ...
        if success['state'] != 'SUCCEEDED':
            ...
            receipt['resumed_successful_job'] = True
        ...
        for gateway in args.gateways:
            ...
            for _ in range(200):
                ...
                for item in page['jobs']:
                    ...
                    seen.append(item['job_id']); ordering.append((datetime.datetime.fromisoformat(item['created_at'].replace('Z', '+00:00')), item['job_id']))
            ...
            if request['output_spec'] == 'h3-native-av-1344x768-5s-24fps':
                bad.append(('native_batch_unsupported', dict(request, generation_count=2)))
                for name, field, value in [('native_long_duration', 'duration_seconds', 10),
                                           ('native_short_duration', 'duration_seconds', 4),
                                           ('native_portrait', 'aspect_ratio', '9:16')]:
                    mismatch = copy.deepcopy(request)
                    mismatch['h3'].setdefault('target', {})[field] = value
        ...
        canceled_worker = sql("select l.worker_instance_id from stage_allocations l join stage_runs r on r.id=l.stage_run_id join attempts a on a.id=r.attempt_id where a.job_id='" + str(uuid.UUID(cancel_id)) + "' and r.stage_key='dit' order by l.allocated_at desc limit 1;")
        ...
        canceled_at = datetime.datetime.fromisoformat(cancellation['decided_at'].replace('Z', '+00:00')).isoformat()
        for gateway in args.gateways:
            target[0] = gateway
