...
cluster_name=${H3_DISPOSABLE_CLUSTER_NAME:-vela-h3-disposable-$(date -u +%Y%m%d%H%M%S)-$$}
...
cleanup() {
	if [ "$created" = true ]; then
		...
		if [ "$retain_cluster" = 0 ]; then
			case "$cluster_name" in
			vela-h3-disposable-*)
				"$k3d_binary" cluster delete "$cluster_name" >/dev/null 2>&1 || true
				;;
			esac
		fi
	fi
	find "$work_directory" -depth -mindepth 1 -delete 2>/dev/null || true
	rmdir "$work_directory" 2>/dev/null || true
}
trap cleanup EXIT HUP INT TERM

for command in "$k3d_binary" "$kubectl_binary" "$docker_binary" openssl; do
	if ! command -v "$command" >/dev/null 2>&1; then
		echo "required command is unavailable: $command" >&2
		exit 1
	fi
done
if "$k3d_binary" cluster list --no-headers 2>/dev/null | awk '{print $1}' | grep -Fx "$cluster_name" >/dev/null; then
	echo "refusing to reuse existing k3d cluster $cluster_name" >&2
	exit 1
fi

docker_architecture=$("$docker_binary" info --format '{{.Architecture}}')
...
"$k3d_binary" cluster create "$cluster_name" \
	--servers 1 \
	--agents 3 \
	--image "$k3s_image" \
	--no-lb \
