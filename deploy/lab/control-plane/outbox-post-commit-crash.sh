...
previous_scenario_job_id=36cdaaf0-c8a1-4c71-93eb-4a89bfb55c0b
previous_scenario_receipt_sha256=9d1ca447fc0c37c16811fc7baf63051d98ea7914f400e01db9b16af25e553ec0
previous_scenario_harness_sha256=cc6a79dad257ad51933cc31b0f664977f4f22d56beacc2b6ead3b9e2f5ec7d80
previous_scenario_receipt=/root/vela-lab-deploy-bc590e20/receipts/process-kill-v1
publisher_tick=1m
...
fail() {
	printf 'outbox-post-commit-crash: %s\n' "$*" >&2
	exit 1
}
...
validate_previous_scenario_receipt() {
	...
	[ "$(cat "$previous_scenario_receipt/STATUS")" = 'status=LAB_REHEARSAL_PASS production_gates=0/9 fixed_scenarios=3/10' ] ||
		fail "previous fixed-scenario STATUS is not a three-scenario lab pass"
	jq -e \
	  ...
	  and .fixed_scenarios_completed == 3
	...
	jq -e \
		--arg process_job_id "$previous_scenario_job_id" '
	  ...
	  and any(.scenarios[];
	    .id == "process-kill"
	    and .status == "LAB_REHEARSAL_PASS"
	    and .job_id == $process_job_id)
	  and any(.scenarios[];
	    .id == "outbox-post-commit-crash" and .status == "NOT_RUN")
...
restore_publisher_config() {
	...
	value=$(printf '%s\n' "$current" | jq -r '.data.VELA_PUBLISHER_TICK // ""')
	case "$value" in
		"") return 0 ;;
		"$publisher_tick") ;;
		*) return 1 ;;
	...
	patch=$(jq -n \
		...
		--arg value "$publisher_tick" '[
		  {op:"test",path:"/metadata/uid",value:$uid},
		  {op:"test",path:"/metadata/resourceVersion",value:$resource_version},
		  {op:"test",path:"/data/VELA_PUBLISHER_TICK",value:$value},
		  {op:"remove",path:"/data/VELA_PUBLISHER_TICK"}
		]')
...
apply_publisher_delay() {
	...
	jq -e '
	  ...
	  and (.data | has("VELA_PUBLISHER_TICK") | not)
	...
	patch=$(jq -n \
		...
		--arg value "$publisher_tick" '[
		  {op:"test",path:"/metadata/uid",value:$uid},
		  {op:"test",path:"/metadata/resourceVersion",value:$resource_version},
		  {op:"add",path:"/data/VELA_PUBLISHER_TICK",value:$value}
		]')
	$kubectl_bin patch configmap vela-lab-control-runtime --namespace "$namespace" \
		--type=json -p "$patch" >/dev/null
	$kubectl_bin get configmap vela-lab-control-runtime --namespace "$namespace" \
		-o json >"$temporary/control-runtime-delayed.json"
	jq -e --arg uid "$uid" --arg tick "$publisher_tick" '
	  .metadata.uid == $uid and .data.VELA_PUBLISHER_TICK == $tick
	' "$temporary/control-runtime-delayed.json" >/dev/null ||
		fail "Publisher delay ConfigMap mutation did not converge"
	printf 'applied_at=%s value=%s\n' "$(date -u '+%Y-%m-%dT%H:%M:%SZ')" "$publisher_tick" \
		>"$temporary/CONFIG_DELAY_APPLIED"
}
...
print(
    "schema=vela-lab-control-outbox-crash-v1 signal=SIGKILL "
...
recover_environment() {
	...
	restore_publisher_config >>"$log_file" 2>&1 || recovery_result=1
	if [ -f "$temporary/CONFIG_DELAY_APPLIED" ] && [ ! -f "$temporary/CONFIG_DEFAULT_RELOADED" ]; then
		if wait_ready_control_pod "$temporary/recovery-control-before.json" 2>>"$log_file"; then
			if reload_control "$temporary/recovery-control-before.json" \
				...
				printf 'reloaded_at=%s\n' "$(date -u '+%Y-%m-%dT%H:%M:%SZ')" \
					>"$temporary/CONFIG_DEFAULT_RELOADED"
			else
				recovery_result=1
			fi
		else
...
cleanup() {
	...
	if [ "$committed" != true ] && [ -n "$temporary" ] && [ -d "$temporary" ]; then
		...
		if recover_environment "$temporary/cleanup.log"; then
			...
			printf 'outbox-post-commit-crash: immediate recovery incomplete; armed watchdog retained\n' >&2
		fi
		printf 'outbox-post-commit-crash: diagnostic receipt preserved at %s\n' "$temporary" >&2
	fi
	exit "$cleanup_result"
}
...
export KUBECONFIG="$kubeconfig"
temporary=$(mktemp -d "$(dirname "$output")/.vela-lab-outbox-post-commit-crash.XXXXXX")
...
apply_publisher_delay
reload_control "$temporary/control-before.json" "$temporary/control-delayed.json" ||
	fail "control Pod did not reload the Publisher delay"
delayed_identity=$(control_identity "$temporary/control-delayed.json")
delayed_uid=$(printf '%s\n' "$delayed_identity" | cut -d '|' -f 2)
delayed_started_at=$(printf '%s\n' "$delayed_identity" | cut -d '|' -f 5)
started_epoch=$(date -d "$delayed_started_at" +%s)
age=$(( $(date +%s) - started_epoch ))
[ "$age" -ge 0 ] && [ "$age" -le 20 ] || fail "delayed Publisher control Pod is outside the bounded startup window"
sleep 2
heartbeat
[ "$(query_database 'SELECT count(*) FROM outbox_events WHERE published_at IS NULL OR claim_token IS NOT NULL;')" = 0 ] ||
	fail "delayed Publisher startup did not drain the baseline Outbox"
...
SELECT job.state,
  ...
  outbox.event_type, outbox.publish_attempts,
...
case "$outbox_before" in
	'QUEUED|0|0|job.ready|0|t|t|t' | \
	'ASSIGNED|1|1|job.ready|0|t|t|t' | \
	'RUNNING|1|1|job.ready|0|t|t|t' | \
	'FINALIZING|1|1|job.ready|0|t|t|t') ;;
	*) fail "Outbox event did not remain in the exact post-commit/pre-claim boundary" ;;
esac
query_database "
...
run_fault_pod "$temporary/control-delayed.json" || fail "control SIGKILL injector did not complete"
fault_receipt=$(grep -E '^schema=vela-lab-control-outbox-crash-v1 signal=SIGKILL signal_transport=pidfd_send_signal container_id=[0-9a-f]{64} old_pid=[0-9]+ old_start_ticks=[0-9]+ signal_at=[^[:space:]]+ production_gates=0/9$' \
...
[ "$old_container_id" = "$(printf '%s\n' "$delayed_identity" | cut -d '|' -f 3)" ] ||
	fail "fault receipt does not bind the delayed Publisher container"

wait_restarted_control_container "$temporary/control-after-crash.json" "$temporary/control-delayed.json" ||
...
before_restarts=$(printf '%s\n' "$delayed_identity" | cut -d '|' -f 4)
after_restarts=$(printf '%s\n' "$after_identity" | cut -d '|' -f 4)
[ "$after_uid" = "$delayed_uid" ] || fail "control Pod identity changed instead of restarting its container"
...
SELECT event_type, publish_attempts,
  ...
  broker_stream, broker_sequence > 0,
  claimed_by IS NULL AND claim_token IS NULL AND claim_expires_at IS NULL
FROM outbox_events
WHERE aggregate_id = '$job_id'::uuid AND event_type = 'job.ready';")
printf '%s\n' "$outbox_after" >"$temporary/outbox-after-recovery.txt"
[ "$outbox_after" = 'job.ready|1|t|VELA_EVENTS|t|t' ] ||
	fail "recovered Outbox event does not prove one post-crash publication"
...
restore_publisher_config || fail "Publisher tick ConfigMap could not be restored"
printf 'restored_at=%s\n' "$(date -u '+%Y-%m-%dT%H:%M:%SZ')" >"$temporary/CONFIG_DELAY_REMOVED"
wait_ready_control_pod "$temporary/control-before-default-reload.json" ||
	fail "control was not Ready before default Publisher reload"
reload_control "$temporary/control-before-default-reload.json" "$temporary/control-default-restored.json" ||
	fail "control did not reload the default Publisher tick"
printf 'reloaded_at=%s\n' "$(date -u '+%Y-%m-%dT%H:%M:%SZ')" >"$temporary/CONFIG_DEFAULT_RELOADED"
...
jq -e --arg uid "$(jq -er '.metadata.uid' "$temporary/control-runtime-before.json")" '
  .metadata.uid == $uid and (.data | has("VELA_PUBLISHER_TICK") | not)
' "$temporary/control-runtime-restored.json" >/dev/null || fail "default Publisher ConfigMap boundary was not restored"
...
jq \
  ...
  (.scenarios[] | select(.id == "outbox-post-commit-crash")) = {
    id:"outbox-post-commit-crash",
    status:"LAB_REHEARSAL_PASS",
    job_id:$job_id,
    started_at:$started_at,
    completed_at:$completed_at,
    fault:"CONTROL_PROCESS_SIGKILL_AFTER_OUTBOX_COMMIT_BEFORE_CLAIM",
...
jq -e --arg job_id "$job_id" '
  ...
  and any(.scenarios[]; .id == "outbox-post-commit-crash" and .job_id == $job_id and .status == "LAB_REHEARSAL_PASS")
' "$temporary/scenario-matrix.json" >/dev/null || fail "four-scenario matrix is invalid"
...
jq -n \
	...
	--argjson restart_count "$after_restarts" '
  {
    ...
    fault:{
      kind:"CONTROL_PROCESS_SIGKILL_AFTER_OUTBOX_COMMIT_BEFORE_CLAIM",
    ...
    outbox:{event_type:"job.ready",publish_attempts_before:0,publish_attempts_after:1,broker_stream:"VELA_EVENTS"},
    ...
    artifacts:[
      ...
      "outbox-before-crash.txt", "outbox-after-recovery.txt",
      "fault-pod-identity.txt", "fault-injection.log", "raw-event-payloads.jsonl",
      "control-runtime-before.json", "control-runtime-delayed.json",
      "control-runtime-restored.json", "control-delayed.json",
      "control-after-crash.json", "control-default-restored.json", "smoke-receipt.json"
    ]
  }
' >"$temporary/summary.json"
printf 'status=LAB_REHEARSAL_PASS production_gates=0/9 fixed_scenarios=4/10\n' >"$temporary/STATUS"
...
temporary=
committed=true
printf 'schema=vela-lab-outbox-post-commit-crash-wrapper-v1 output=%s job=%s result=LAB_REHEARSAL_PASS fixed_scenarios=4/10 production_gates=0/9\n' \
	"$output" "$job_id"
