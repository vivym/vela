...
previous_scenario_job_id=af71a549-36be-49b3-aaba-e7c299245f92
previous_scenario_receipt_sha256=d73cdce02500580a4f1b5961844e7808f5f19f3cb4bc0bb9be25d236a9165bfb
previous_scenario_harness_sha256=cc37ee0df5e813e0929f4ea083782d785153b846bd81040d70802f397065f0a0
previous_scenario_receipt=/root/vela-lab-deploy-bc590e20/receipts/publisher-post-puback-pre-mark-crash-v2
fault_phase=publisher-pre-puback-crash
...
fail() {
	printf 'publisher-pre-puback-crash: %s\n' "$*" >&2
	exit 1
}
...
fetch_nats_monitor() {
	...
	if ! curl --connect-timeout 2 --max-time 5 --fail --silent --show-error \
		"http://127.0.0.1:$fetch_local_port/jsz?streams=true&config=true" >"$fetch_destination"; then
		stop_nats_port_forward
		return 1
	fi
	stop_nats_port_forward
...
capture_nats_stream_state() {
	...
	state_raw=$state_destination.raw
	...
	printf '%s\n' "$state_leader" | grep -Eq '^vela-lab-nats-[0-2]$' || return 1
	if [ "$state_leader" = vela-lab-nats-0 ]; then
		mv "$state_discovery" "$state_raw"
	else
		fetch_nats_monitor "$state_leader" "$state_raw" "$state_log" || return 1
	...
	jq -e '
	  ...
	  | {
	      schema:"vela-lab-nats-stream-state-v1",
	      server_id:.server_id,
	      captured_at:.now,
	      stream:($streams[0] | {name, config, cluster, state})
	    }
	' "$state_raw" >"$state_destination" || return 1
	rm -f -- "$state_raw"
	jq -e --arg leader "$state_leader" '
	  .schema == "vela-lab-nats-stream-state-v1"
	  ...
	  and .stream.cluster.leader == $leader
...
validate_previous_scenario_receipt() {
	...
	[ "$(cat "$previous_scenario_receipt/STATUS")" = 'status=LAB_REHEARSAL_PASS production_gates=0/9 fixed_scenarios=5/10' ] ||
		fail "previous fixed-scenario STATUS is not a five-scenario lab pass"
	jq -e \
	  ...
	  and .production_gates == "0/9"
		  and .fixed_scenarios_completed == 5
	...
	jq -e \
	  ...
	  and ([.scenarios[].id] | sort) == ([
	    ...
	    "stale-fence-late-completion"
		  ...
		  and any(.scenarios[];
		    .id == "publisher-pre-puback-crash" and .status == "NOT_RUN")
...
cleanup() {
	...
	if [ "$committed" != true ] && [ -n "$temporary" ] && [ -d "$temporary" ]; then
		...
		if recover_environment "$temporary/cleanup.log"; then
			...
			printf 'publisher-pre-puback-crash: immediate recovery incomplete; armed watchdog retained\n' >&2
		fi
		printf 'publisher-pre-puback-crash: diagnostic receipt preserved at %s\n' "$temporary" >&2
	fi
	exit "$cleanup_result"
}
trap 'cleanup $?' EXIT
...
export KUBECONFIG="$kubeconfig"
temporary=$(mktemp -d "$(dirname "$output")/.vela-lab-publisher-pre-puback-crash.XXXXXX")
...
while [ "$iteration" -lt 30 ]; do
	...
	if $kubectl_bin exec --namespace "$namespace" "pod/$control_pod_name" --container control -- \
		/usr/local/bin/vela-control --lab-read-outbox-fault-marker \
		>"$temporary/pre-puback-marker.json" 2>"$temporary/pre-puback-marker-read-error.txt"; then
		break
	fi
	iteration=$((iteration + 1))
	sleep 1
done
[ -s "$temporary/pre-puback-marker.json" ] || fail "Publisher pre-PubAck marker was not observed through kubectl exec"
rm -f -- "$temporary/pre-puback-marker-read-error.txt"
jq -e --arg phase "$fault_phase" '
  ...
  and .schema == "vela-lab-outbox-fault-marker-v1"
	...
	' "$temporary/pre-puback-marker.json" >/dev/null || fail "Publisher pre-PubAck marker is invalid"
marker_event_id=$(jq -er '.event_id' "$temporary/pre-puback-marker.json")
...
case "$outbox_before" in
	...
	*) fail "Outbox event did not remain at the exact pre-PubAck boundary" ;;
esac
...
printf '%s\n' "$fault_window" | grep -Eq '^t\|t\|[0-9]+$' ||
	fail "SIGKILL did not occur inside the bounded pre-PubAck hook window"
fault_elapsed_seconds=$(printf '%s\n' "$fault_window" | cut -d '|' -f 3)
[ "$fault_elapsed_seconds" -lt 90 ] ||
	fail "SIGKILL exceeded the bounded pre-PubAck hook window"
...
SELECT event_type, publish_attempts,
	  ...
	  published_at IS NOT NULL AND published_at < '$database_started_at'::timestamptz + interval '10 minutes',
...
WHERE aggregate_id = '$job_id'::uuid
	...
	if printf '%s\n' "$outbox_after" | grep -Eq '^job\.ready\|2\|t\|t\|VELA_EVENTS\|[1-9][0-9]*\|t$'; then
		break
	fi
...
printf '%s\n' "$outbox_after" | grep -Eq '^job\.ready\|2\|t\|t\|VELA_EVENTS\|[1-9][0-9]*\|t$' ||
	fail "recovered Outbox event lacks the delegated Publisher receipt"
marker_stream=$(printf '%s\n' "$outbox_after" | cut -d '|' -f 5)
marker_sequence=$(printf '%s\n' "$outbox_after" | cut -d '|' -f 6)
precrash_sequence=$(jq -er '.stream.state.last_seq' "$temporary/nats-stream-before-crash.json")
[ "$marker_sequence" -gt "$precrash_sequence" ] ||
	fail "recovered Publisher sequence was already present before the pre-PubAck crash"
capture_nats_stream_state "$temporary/nats-stream-after-recovery.json" ||
	fail "could not capture the recovered VELA_EVENTS stream state"
jq -e --argjson sequence "$marker_sequence" '
  .stream.state.last_seq >= $sequence
' "$temporary/nats-stream-after-recovery.json" >/dev/null ||
	fail "VELA_EVENTS does not contain the recovered Publisher sequence"
printf 'schema=vela-lab-pre-puback-publication-check-v1 event_id=%s pre_crash_last_sequence=%s recovered_event_sequence=%s result=ABSENT_BEFORE_CRASH\n' \
	"$marker_event_id" "$precrash_sequence" "$marker_sequence" \
	>"$temporary/nats-pre-puback-publication-check.txt"
...
jq \
  ...
  (.scenarios[] | select(.id == "publisher-pre-puback-crash")) = {
    id:"publisher-pre-puback-crash",
    status:"LAB_REHEARSAL_PASS",
    job_id:$job_id,
    started_at:$started_at,
    completed_at:$completed_at,
    fault:"CONTROL_PROCESS_SIGKILL_BEFORE_PUBLISHER_DELEGATED_PUBLISH",
...
jq -e --arg job_id "$job_id" '
  ...
  and any(.scenarios[]; .id == "publisher-pre-puback-crash" and .job_id == $job_id and .status == "LAB_REHEARSAL_PASS")
' "$temporary/scenario-matrix.json" >/dev/null || fail "six-scenario matrix is invalid"
...
nats_messages_before=$(jq -er '.stream.state.messages' "$temporary/nats-stream-before-job.json")
nats_last_sequence_before=$(jq -er '.stream.state.last_seq' "$temporary/nats-stream-before-job.json")
nats_messages_before_crash=$(jq -er '.stream.state.messages' "$temporary/nats-stream-before-crash.json")
nats_last_sequence_before_crash=$(jq -er '.stream.state.last_seq' "$temporary/nats-stream-before-crash.json")
nats_messages_after=$(jq -er '.stream.state.messages' "$temporary/nats-stream-after-recovery.json")
nats_last_sequence_after=$(jq -er '.stream.state.last_seq' "$temporary/nats-stream-after-recovery.json")
jq -n \
	--arg job_id "$job_id" \
	--arg started_at "$database_started_at" \
	--arg completed_at "$completed_at" \
	...
	--argjson nats_messages_before "$nats_messages_before" \
	--argjson nats_last_sequence_before "$nats_last_sequence_before" \
	--argjson nats_messages_before_crash "$nats_messages_before_crash" \
	--argjson nats_last_sequence_before_crash "$nats_last_sequence_before_crash" \
	--argjson nats_messages_after "$nats_messages_after" \
	--argjson nats_last_sequence_after "$nats_last_sequence_after" \
	--argjson restart_count "$after_restarts" '
  {
    ...
    fault:{
      kind:"CONTROL_PROCESS_SIGKILL_BEFORE_PUBLISHER_DELEGATED_PUBLISH",
    ...
    outbox:{
	  ...
	  broker_stream_before_crash:"",
	  broker_sequence_before_crash:0,
      broker_stream:$broker_stream,
      broker_sequence:$broker_sequence,
	  delegated_publish_absent_before_crash:true,
	  delegated_publish_after_recovery:true,
	  recovery_within_duplicate_window:true
	},
	nats:{
	  messages_before_job:$nats_messages_before,
	  messages_before_crash:$nats_messages_before_crash,
	  last_sequence_before_job:$nats_last_sequence_before,
	  last_sequence_before_crash:$nats_last_sequence_before_crash,
	  duplicate_window_nanoseconds:600000000000,
	  messages_after_recovery:$nats_messages_after,
	  last_sequence_after_recovery:$nats_last_sequence_after
    },
    ...
    artifacts:[
      "scenario-matrix.json", "authority-before.json", "authority-after.json",
	      ...
	      "pre-puback-marker.json", "nats-pre-puback-publication-check.txt",
	  "nats-stream-before-job.json", "nats-stream-before-crash.json",
	  "nats-stream-after-recovery.json", "fault-window.txt",
...
printf 'status=LAB_REHEARSAL_PASS production_gates=0/9 fixed_scenarios=6/10\n' >"$temporary/STATUS"
...
temporary=
committed=true
printf 'schema=vela-lab-publisher-pre-puback-crash-wrapper-v1 output=%s job=%s result=LAB_REHEARSAL_PASS fixed_scenarios=6/10 production_gates=0/9\n' \
	"$output" "$job_id"
