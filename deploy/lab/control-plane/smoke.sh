...
production_gate_receipts=$($kubectl_bin exec --namespace "$namespace" statefulset/vela-lab-postgres -- \
	sh -ec 'PGPASSWORD=$POSTGRES_PASSWORD psql --no-psqlrc --tuples-only --no-align --username "$POSTGRES_USER" --dbname "$POSTGRES_DB" --command "SELECT count(*) FROM production_gate_receipts"')
