...
expected_wrapper_sha256=a313501ed2ad04a129e10f9e47b0909530753dd9ae5e74c38c20fbaab043c7d1
expected_helper_sha256=6908db20c090f65501210bcd9093c635224c6301175877fa44258f8d8621e070
...
fail() {
	printf 'upgrade-mock-runner-failure-mode: %s\n' "$*" >&2
	exit 1
}
...
install_file() {
	...
	destination=$2
	mode=$3
	temporary=$(mktemp "$(dirname "$destination")/.upgrade.XXXXXX")
	install -m "$mode" -o 0 -g 0 "$source" "$temporary"
	mv -f -- "$temporary" "$destination"
}
...
rollback() {
	...
	if [ "$upgraded" != true ] && [ -n "$backup" ] && [ -d "$backup" ]; then
		...
		restore_file "$backup/mock-backend-wrapper.sh" "$root/config/mock-backend-wrapper.sh" 0555 || true
		restore_file "$backup/set-mock-runner-mode.sh" "$root/admin/set-mock-runner-mode.sh" 0550 || true
		if [ "$stopped" = true ] && [ -n "$image" ]; then
			if "$root/admin/start-mock-runner-container.sh" "$image" mode-control >/dev/null 2>&1; then
				printf 'upgrade-mock-runner-failure-mode: rollback restored the previous Runner\n' >&2
			else
				printf 'upgrade-mock-runner-failure-mode: rollback could not restart the previous Runner\n' >&2
			fi
		fi
	fi
...
backup=$(mktemp -d "$root/.failure-mode-upgrade.XXXXXX")
chmod 0700 "$backup"
...
backup=
trap - EXIT HUP INT TERM
printf 'schema=vela-lab-mock-runner-failure-mode-upgrade-v1 result=PASS host=%s lan_ip=%s image=%s mode=success wrapper_sha256=%s helper_sha256=%s production_gates=0/9\n' \
