...
func TestValidationPIDFDOfferHelper(t *testing.T) {
...
func TestValidationPIDFDOfferBindsKernelSenderToTask(t *testing.T) {
	if os.Geteuid() != 0 {
	...
	for _, scenario := range []string{"self", "wrong-task", "wrong-uid", "regular-file", "different-process"} {
		t.Run(scenario, func(t *testing.T) {
			...
			offer, err := newPIDFDOfferListener(directory, "worker", uid, 65532)
			if err != nil {
				t.Fatal(err)
			}
			defer offer.Close()
			...
			command := exec.Command(binary, "-test.run=^TestValidationPIDFDOfferHelper$")
...
func TestValidationPIDFDOfferCanceledAccept(t *testing.T) {
	if os.Geteuid() != 0 {
		t.Skip("requires root-owned socket")
	}
	offer, err := newPIDFDOfferListener(t.TempDir(), "runtime", 65532, 65532)
	if err != nil {
		t.Fatal(err)
	}
	defer offer.Close()
