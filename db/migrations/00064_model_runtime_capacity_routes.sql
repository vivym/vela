...
DECLARE
    ...
    v_replay boolean;
    v_seen_route_ids uuid[] := ARRAY[]::uuid[];
    v_routes_match boolean;
BEGIN
    -- The legacy apply function has its own replay path, so the wrapper must
    -- make the new/replay decision under one plan-scoped transaction lock.
    PERFORM pg_catalog.pg_advisory_xact_lock(
        pg_catalog.hashtextextended(v_plan_id::text, 640064)
    );
    SELECT EXISTS (
        SELECT 1
    ...
    LOOP
        ...
        LOOP
            ...
            IF (v_route ->> 'model_residency_id')::uuid = ANY(v_seen_route_ids) THEN
                RAISE EXCEPTION USING
                    ERRCODE = '23514',
                    CONSTRAINT = 'model_runtime_capacity_route_duplicate',
                    MESSAGE = 'ResidencyPlan contains a duplicate ModelRuntime route';
            END IF;
            v_seen_route_ids := array_append(
                v_seen_route_ids,
    ...
    WITH submitted AS (
        SELECT
            (route ->> 'model_residency_id')::uuid AS model_residency_id,
            (worker ->> 'id')::uuid AS worker_instance_id,
            (route ->> 'capacity_pool_id')::uuid AS capacity_pool_id,
            (route ->> 'stage_profile_revision_id')::uuid AS stage_profile_revision_id,
            v_result.plan_revision_id AS residency_plan_revision_id
        FROM jsonb_array_elements(p_plan -> 'worker_instances') AS workers(worker)
        CROSS JOIN LATERAL jsonb_array_elements(
            worker -> 'model_runtime_routes'
        ) AS routes(route)
    ), persisted AS (
        SELECT
            route.model_residency_id,
            route.worker_instance_id,
            route.capacity_pool_id,
            route.stage_profile_revision_id,
            route.residency_plan_revision_id
        FROM public.model_runtime_capacity_routes AS route
        WHERE route.residency_plan_revision_id = v_result.plan_revision_id
    )
    SELECT
        NOT EXISTS (SELECT * FROM submitted EXCEPT SELECT * FROM persisted)
        AND NOT EXISTS (SELECT * FROM persisted EXCEPT SELECT * FROM submitted)
    INTO v_routes_match;
    IF NOT coalesce(v_routes_match, false) THEN
        RAISE EXCEPTION USING
            ERRCODE = '55000',
            CONSTRAINT = 'model_runtime_capacity_route_set_replay_conflict',
            MESSAGE = 'ResidencyPlan ModelRuntime route set does not match persisted authority';
    END IF;
