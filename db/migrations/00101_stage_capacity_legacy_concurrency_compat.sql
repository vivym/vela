...
-- +goose StatementBegin
CREATE OR REPLACE FUNCTION public.vela_lock_stage_graph_ready_capacity_path(p_execution_graph_revision_id uuid, p_execution_profile_revision_id uuid)
 RETURNS TABLE(ready boolean, retry_after_seconds integer)
 LANGUAGE plpgsql
 SECURITY DEFINER
              ...
              JOIN public.model_residencies AS residency
               ...
               AND EXISTS (
                   ...
                   WHERE route.worker_instance_id = worker.id
                     ...
                     AND route.stage_profile_revision_id =
                         option.stage_profile_revision_id
               )
               AND residency.state = 'READY'
              WHERE option.execution_graph_revision_id =
                    graph_stage.execution_graph_revision_id
