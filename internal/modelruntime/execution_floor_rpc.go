...
func (supervisor *Supervisor) InstallStageExecutionFloor(
	...
	if request == nil || (request.GetSchemaVersion() != 1 && request.GetSchemaVersion() != 2) || request.GetIdentity() == nil ||
	...
	response.SchemaVersion = request.GetSchemaVersion()
	...
	installation, err := supervisor.installExecutionFloor(ctx, request.GetDisposition(), request.GetSchemaVersion() == 2)
	if err != nil {
