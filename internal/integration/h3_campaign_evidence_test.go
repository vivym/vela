	...
	if err := database.Admin.QueryRow(`
		SELECT
			...
			AND to_regprocedure(
				...
				AND to_regprocedure('vela_admit_h3_exact_cache_entry(jsonb)') IS NULL
				AND to_regclass('h3_exact_cache_admission_receipts') IS NULL
				AND to_regclass('h3_exact_cache_hit_cursors') IS NULL
		`).Scan(&surfaceRemoved); err != nil {
		t.Fatalf("inspect contracted H3 exact cache reconciliation surface: %v", err)
	}
	if !surfaceRemoved {
		t.Fatal("H3 exact cache reconciliation surface survived migration Down")
	}
...
func assertH3ExactCacheReconciliationRoleSurface(t *testing.T, database testDatabase) {
	t.Helper()
	var readerOwner, finderOwner, admitOwner string
	var coordinatorCanRead, coordinatorCanFind, coordinatorCanAdmit bool
	var internalCanRead, internalCanFind, internalCanAdmit bool
	if err := database.Admin.QueryRow(`
		SELECT
			...
			COALESCE(has_function_privilege(
				...
				COALESCE(pg_get_userbyid(admitter.proowner), ''),
				COALESCE(has_function_privilege(
					'vela_attempt_coordinator_login', admitter.oid, 'EXECUTE'
				), false),
				COALESCE(has_function_privilege(
					'vela_internal_login', admitter.oid, 'EXECUTE'
				), false)
		FROM (VALUES (
		...
		CROSS JOIN (VALUES (
			...
			CROSS JOIN (VALUES (
				to_regprocedure('vela_admit_h3_exact_cache_entry(jsonb)')
			)) AS admit_function(oid)
			LEFT JOIN pg_proc AS admitter ON admitter.oid = admit_function.oid
		`).Scan(
		&readerOwner, &finderOwner,
		&coordinatorCanRead, &coordinatorCanFind,
		&internalCanRead, &internalCanFind,
		&admitOwner, &coordinatorCanAdmit, &internalCanAdmit,
	); err != nil {
	...
	if readerOwner != "vela_attempt_coordinator_owner" ||
		...
		admitOwner != "vela_attempt_coordinator_owner" ||
		!coordinatorCanRead || !coordinatorCanFind || !coordinatorCanAdmit ||
		internalCanRead || internalCanFind || internalCanAdmit {
		t.Fatalf(
			"H3 exact cache role surface owners=%s/%s/%s coordinator=%t/%t/%t internal=%t/%t/%t",
			readerOwner, finderOwner, admitOwner,
			coordinatorCanRead, coordinatorCanFind, coordinatorCanAdmit,
			internalCanRead, internalCanFind, internalCanAdmit,
		)
	}
...
func writeH3CampaignReleaseBundle(t *testing.T) string {
	...
	render := catalogReleaseRender("fleet-controller", controlImage)
