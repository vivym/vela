...
func TestH3ExactCacheAdmissionReceiptPreventsDeduplicatedArtifactStarvation(t *testing.T) {
	database, coordinator, serverURL := newH3IntegrationEnvironment(t)
	if _, err := database.Admin.Exec(`
		UPDATE projects SET running_limit = 10 WHERE id = $1
	`, testProjectID); err != nil {
		t.Fatalf("raise mock H3 receipt Project concurrency: %v", err)
	}
	seedWorkerRegistryPlan(t, database.Admin)
	registry, err := fleet.NewService(newRolePool(
		t, database.DSN, "vela_fleet_login", "vela-fleet-password",
	...
	if err != nil {
		t.Fatalf("construct H3 receipt Worker Registry: %v", err)
	}
	artifacts, err := stageartifact.NewPostgresRepository(newRolePool(
		t, database.DSN, "vela_stage_artifact_login", "vela-stage-artifact-password",
	))
	if err != nil {
		t.Fatalf("construct H3 receipt StageArtifact repository: %v", err)
	}
	cache, err := stagecache.NewPostgresRepository(newRolePool(
		t, database.DSN, "vela_attempt_coordinator_login", "vela-attempt-coordinator-password",
	))
	if err != nil {
		t.Fatalf("construct H3 receipt Stage Cache repository: %v", err)
	}
	now := time.Now().UTC().Truncate(time.Millisecond)
	if _, err := cache.SetProjectControl(context.Background(), stagecache.ProjectControlCommand{
		...
		t.Fatalf("enable H3 receipt Project Stage Cache: %v", err)
	}

	objectStore := artifactstore.NewLocal()
	first := materializeMockH3ExactEncoder(
		t, database, coordinator, serverURL, registry, artifacts, objectStore,
		"h3-receipt-first", "h3-receipt-node-first", 0xa1,
	)
	second := materializeMockH3ExactEncoder(
		t, database, coordinator, serverURL, registry, artifacts, objectStore,
		"h3-receipt-second", "h3-receipt-node-second", 0xa2,
	)
	cacheKey := sha256.Sum256([]byte("project/exact/encoder/receipt-starvation-v1"))
	admit := func(artifact stageartifact.Artifact) stagecache.AdmitDecision {
		t.Helper()
		decision, admitErr := cache.AdmitH3Exact(context.Background(), stagecache.AdmitCommand{
			CommandID: uuid.New(), EntryID: uuid.New(), ArtifactID: artifact.ID,
			CachePolicyRevisionID:       uuid.MustParse(h3CachePolicyID),
			StageProfileRevisionID:      uuid.MustParse(encoderStageProfileID),
			ResultEquivalenceRevisionID: uuid.MustParse(h3EncoderEquivalence),
			Scope:                       stagecache.ScopeProject,
			StageKey:                    "encoder",
		...
		if admitErr != nil {
			t.Fatalf("admit H3 exact receipt artifact %s: %v", artifact.ID, admitErr)
		}
		return decision
	}
	firstAdmission := admit(first)
	secondAdmission := admit(second)
	if secondAdmission.EntryID != firstAdmission.EntryID || !secondAdmission.Deduplicated {
		t.Fatalf("deduplicated H3 admissions first=%#v second=%#v", firstAdmission, secondAdmission)
	}

	var receiptCount int
	if err := database.Admin.QueryRow(`
		SELECT count(*)
		FROM h3_exact_cache_admission_receipts
		WHERE stage_artifact_id = ANY($1::uuid[])
	`, []uuid.UUID{first.ID, second.ID}).Scan(&receiptCount); err != nil {
		t.Fatalf("count H3 exact admission receipts: %v", err)
	}
	if receiptCount != 2 {
		t.Fatalf("H3 exact admission receipts = %d, want 2", receiptCount)
	}
	if _, err := database.Admin.Exec(`
		UPDATE stage_cache_entries
		SET state = 'EVICTED', terminal_at = clock_timestamp()
		WHERE id = $1
	`, firstAdmission.EntryID); err != nil {
		t.Fatalf("terminalize deduplicated H3 exact entry: %v", err)
	}

	third := materializeMockH3ExactEncoder(
		t, database, coordinator, serverURL, registry, artifacts, objectStore,
		"h3-receipt-third", "h3-receipt-node-third", 0xa3,
	)
	candidates, err := cache.ReadH3ExactCandidates(context.Background(), "ADMIT", 1)
	if err != nil {
		t.Fatalf("read H3 exact admission candidate after terminal entry: %v", err)
	}
	if len(candidates) != 1 || candidates[0].ArtifactID != third.ID {
		t.Fatalf("H3 exact admission candidate after receipts = %#v, want %s", candidates, third.ID)
	}
}

func TestH3ExactCacheHitCursorPersistsFairPaginationAcrossRepositories(t *testing.T) {
	database, _, serverURL := newH3IntegrationEnvironment(t)
	cache, err := stagecache.NewPostgresRepository(newRolePool(
		t, database.DSN, "vela_attempt_coordinator_login", "vela-attempt-coordinator-password",
	))
	if err != nil {
		t.Fatalf("construct H3 HIT cursor repository: %v", err)
	}
	if _, err := cache.SetProjectControl(context.Background(), stagecache.ProjectControlCommand{
		OrganizationID: uuid.MustParse(testOrganizationID), ProjectID: uuid.MustParse(testProjectID),
		CachePolicyRevisionID: uuid.MustParse(h3CachePolicyID), Enabled: true,
		...
		t.Fatalf("enable H3 HIT cursor Project Stage Cache: %v", err)
	}
	for index := 0; index < 3; index++ {
		instantiateH3IntegrationGraph(
			t, database, serverURL, fmt.Sprintf("h3-hit-cursor-%d", index),
		)
	}

	allCandidates, err := cache.ReadH3ExactCandidates(context.Background(), "HIT", 100)
	if err != nil {
		t.Fatalf("read complete H3 HIT cursor candidate set: %v", err)
	}
	if len(allCandidates) < 3 {
		t.Fatalf("complete H3 HIT cursor candidate set = %#v", allCandidates)
	}
	expected := make(map[string]struct{}, len(allCandidates))
	expectedStageRuns := make(map[uuid.UUID]struct{})
	for _, candidate := range allCandidates {
		expected[candidate.StageRunID.String()+"/"+candidate.StageProfileRevisionID.String()] = struct{}{}
		expectedStageRuns[candidate.StageRunID] = struct{}{}
	}
	if len(expectedStageRuns) != 3 {
		t.Fatalf("H3 HIT cursor StageRuns = %d, want 3", len(expectedStageRuns))
	}

	seen := make(map[string]struct{}, len(expected))
	for index := 0; index < len(expected); index++ {
		reader, repositoryErr := stagecache.NewPostgresRepository(newRolePool(
			t, database.DSN, "vela_attempt_coordinator_login", "vela-attempt-coordinator-password",
		))
		if repositoryErr != nil {
			t.Fatalf("construct H3 HIT cursor reader %d: %v", index, repositoryErr)
		}
		candidates, readErr := reader.ReadH3ExactCandidates(context.Background(), "HIT", 1)
		if readErr != nil {
			t.Fatalf("read H3 HIT cursor page %d: %v", index, readErr)
		}
		if len(candidates) != 1 {
			t.Fatalf("H3 HIT cursor page %d = %#v", index, candidates)
		}
		identity := candidates[0].StageRunID.String() + "/" +
			candidates[0].StageProfileRevisionID.String()
		if _, known := expected[identity]; !known {
			t.Fatalf("H3 HIT cursor returned unknown candidate %s", identity)
		}
		if _, duplicate := seen[identity]; duplicate {
			t.Fatalf("H3 HIT cursor repeated candidate %s before completing rotation", identity)
		}
		seen[identity] = struct{}{}
	}
}

func materializeMockH3ExactEncoder(
	t *testing.T,
	...
	t.Helper()
	job, attemptID := instantiateH3IntegrationGraph(t, database, serverURL, idempotencyKey)
	var stageRunID uuid.UUID
	if err := database.Admin.QueryRow(`
		SELECT id FROM stage_runs WHERE attempt_id = $1 AND stage_key = 'encoder'
	`, attemptID).Scan(&stageRunID); err != nil {
		t.Fatalf("read mock H3 exact Encoder StageRun: %v", err)
	}
	encoder := h3IntegrationStage{
		key: "encoder", stage: "ENCODER",
		profileStableID: "h3-encoder-single-gpu", profileID: encoderStageProfileID,
	...
	assignment := assignH3IntegrationStage(
		t, database, coordinator, registry, attemptID, stageRunID, 1, encoder, identityByte,
	)
	authority := signedAssignedStageAuthority(t, database, job, assignment, 2)
	_ = startH3IntegrationStage(t, database, assignment, authority)
	return materializeH3IntegrationStage(
		t, artifacts, objectStore, attemptID, stageRunID, assignment, encoder,
		[]byte("mock exact reusable encoder conditioning"),
		[]byte(`{"kind":"conditioning","cache":"h3-receipt-mock"}`),
	)
}
