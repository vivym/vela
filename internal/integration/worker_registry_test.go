...
func workerRegistryPoolForSchema(t *testing.T, database testDatabase) *pgxpool.Pool {
	t.Helper()
	var contracted bool
	if err := database.Admin.QueryRow(`
		SELECT to_regprocedure(
			'public.vela_resolve_stage_job_execution_route(uuid,uuid,uuid)'
		) IS NOT NULL
	`).Scan(&contracted); err != nil {
		t.Fatalf("inspect Worker Registry schema authority: %v", err)
	}
	if contracted {
		return newRolePool(t, database.DSN, "vela_fleet_login", "vela-fleet-password")
	}
	return newRolePool(t, database.DSN, "vela_internal_login", "vela-internal-password")
}
...
func TestH3LaunchEvidenceCapturesAuthoritativeRegistrySnapshot(t *testing.T) {
	...
	evidencePool := newRolePool(
		t,
		database.DSN,
		"vela_h3_campaign_evidence_login",
		"vela-h3-campaign-evidence-password",
	)
	reader, err := h3launchevidence.NewPostgresRegistryReader(evidencePool)
	if err != nil {
...
func TestResidencyPlanReplayRejectsModelRuntimeRouteSetChanges(t *testing.T) {
	database := newPostgres(t)
	applyFoundation(t, database.Admin)
	seedAdmissionFixture(t, database.Admin)
	seedStageExecutionCatalog(t, database.Admin)
	...
	service, err := fleet.NewService(fleetPool)
	if err != nil {
		t.Fatalf("construct route replay Fleet service: %v", err)
	}
	proposalID := uuid.MustParse("49200000-0000-0000-0000-000000000280")
	if _, err := service.Propose(context.Background(), fleet.ResidencyPlanInputs{
		ProposalID: proposalID, InputDigest: mustDigestBytes(t, digestHex(0xd0)),
		ConfidencePPM: 900000, ExpiresAt: time.Now().UTC().Add(time.Hour),
		MinCapacity: map[string]int64{"dit": 2}, DesiredCapacity: map[string]int64{"dit": 2},
		MaxCapacity: map[string]int64{"dit": 2}, Cooldown: time.Hour,
		BudgetMicroUnits: 1000000, ReasonCodes: []string{"ROUTE_REPLAY"},
		ProposedBy: "capacity-simulator/route-replay",
	}); err != nil {
		t.Fatalf("record route replay ResidencyProposal: %v", err)
	}

	plan, secondPoolID := approvedResidencyPlanWithSecondRouteFixture(proposalID)
	var approved fleet.ApprovedResidencyPlan
	if err := json.Unmarshal(mustJSON(t, plan), &approved); err != nil {
		t.Fatalf("decode route replay ResidencyPlan: %v", err)
	}
	if _, err := service.Apply(context.Background(), approved); err != nil {
		t.Fatalf("apply route replay ResidencyPlan: %v", err)
	}

	clonePlan := func(t *testing.T) map[string]any {
		t.Helper()
		var clone map[string]any
		if err := json.Unmarshal(mustJSON(t, plan), &clone); err != nil {
			t.Fatalf("clone route replay ResidencyPlan: %v", err)
		}
		return clone
	}
	workers := func(plan map[string]any) []any {
		return plan["worker_instances"].([]any)
	}
	routes := func(worker any) []any {
		return worker.(map[string]any)["model_runtime_routes"].([]any)
	}

	t.Run("added route", func(t *testing.T) {
		replay := clonePlan(t)
		worker := workers(replay)[0].(map[string]any)
		workerRoutes := routes(worker)
		worker["model_runtime_routes"] = append(workerRoutes, map[string]any{
			"model_residency_id":        uuid.New().String(),
			"capacity_pool_id":          secondPoolID,
			"stage_profile_revision_id": "49000000-0000-0000-0000-000000000040",
		})
		_, err := fleetPool.Exec(
			context.Background(), "SELECT * FROM vela_apply_residency_plan($1::jsonb)",
			mustJSON(t, replay),
		)
		assertPostgresConstraint(t, err, "model_runtime_capacity_route_replay_conflict")
	})

	t.Run("removed route", func(t *testing.T) {
		replay := clonePlan(t)
		replay["worker_instances"] = workers(replay)[:1]
		_, err := fleetPool.Exec(
			context.Background(), "SELECT * FROM vela_apply_residency_plan($1::jsonb)",
			mustJSON(t, replay),
		)
		assertPostgresConstraint(t, err, "model_runtime_capacity_route_set_replay_conflict")
	})

	t.Run("reassigned route", func(t *testing.T) {
		replay := clonePlan(t)
		firstRoute := routes(workers(replay)[0])[0].(map[string]any)
		firstRoute["capacity_pool_id"] = secondPoolID
		firstRoute["stage_profile_revision_id"] = "49000000-0000-0000-0000-000000000040"
		_, err := fleetPool.Exec(
			context.Background(), "SELECT * FROM vela_apply_residency_plan($1::jsonb)",
			mustJSON(t, replay),
		)
...
func TestResidencyPlanConcurrentFirstApplySerializesModelRuntimeRoutes(t *testing.T) {
	database := newPostgres(t)
	applyFoundation(t, database.Admin)
	seedAdmissionFixture(t, database.Admin)
	seedStageExecutionCatalog(t, database.Admin)
	fleetPool := newRolePool(t, database.DSN, "vela_fleet_login", "vela-fleet-password")
	proposalID := uuid.MustParse("49200000-0000-0000-0000-000000000290")
	service, err := fleet.NewService(fleetPool)
	if err != nil {
		t.Fatalf("construct concurrent route Fleet service: %v", err)
	}
	if _, err := service.Propose(context.Background(), fleet.ResidencyPlanInputs{
		ProposalID: proposalID, InputDigest: mustDigestBytes(t, digestHex(0xd4)),
		ConfidencePPM: 900000, ExpiresAt: time.Now().UTC().Add(time.Hour),
		MinCapacity: map[string]int64{"dit": 2}, DesiredCapacity: map[string]int64{"dit": 2},
		MaxCapacity: map[string]int64{"dit": 2}, Cooldown: time.Hour,
		BudgetMicroUnits: 1000000, ReasonCodes: []string{"ROUTE_CONCURRENCY"},
		ProposedBy: "capacity-simulator/route-concurrency",
	}); err != nil {
		t.Fatalf("record concurrent route ResidencyProposal: %v", err)
	}

	basePlan, secondPoolID := approvedResidencyPlanWithSecondRouteFixture(proposalID)
	var expandedPlan map[string]any
	if err := json.Unmarshal(mustJSON(t, basePlan), &expandedPlan); err != nil {
		t.Fatalf("clone concurrent route ResidencyPlan: %v", err)
	}
	expandedWorkers := expandedPlan["worker_instances"].([]any)
	expandedWorker := expandedWorkers[0].(map[string]any)
	expandedRoutes := expandedWorker["model_runtime_routes"].([]any)
	expandedWorker["model_runtime_routes"] = append(expandedRoutes, map[string]any{
		"model_residency_id":        uuid.New().String(),
		"capacity_pool_id":          secondPoolID,
		"stage_profile_revision_id": "49000000-0000-0000-0000-000000000040",
	})
	...
	if err != nil {
		t.Fatalf("begin concurrent route blocker: %v", err)
	}
	defer func() { _ = blocker.Rollback() }()
	planID := basePlan["id"].(string)
	if _, err := blocker.Exec(`
		SELECT pg_advisory_xact_lock(hashtextextended($1, 640064))
	`, planID); err != nil {
		t.Fatalf("lock concurrent route plan: %v", err)
	}
	...
	for _, candidate := range []struct {
		name string
		plan map[string]any
	}{{"expanded", expandedPlan}, {"base", basePlan}} {
		connection, acquireErr := fleetPool.Acquire(context.Background())
		if acquireErr != nil {
			t.Fatalf("acquire concurrent route connection: %v", acquireErr)
		}
		connections = append(connections, connection)
		var pid int
		if err := connection.QueryRow(context.Background(), `SELECT pg_backend_pid()`).Scan(&pid); err != nil {
			t.Fatalf("read concurrent route backend PID: %v", err)
		}
		payload := mustJSON(t, candidate.plan)
		go func(name string, connection *pgxpool.Conn, payload []byte) {
			_, applyErr := connection.Exec(
				context.Background(),
				"SELECT * FROM vela_apply_residency_plan($1::jsonb)", payload,
			)
			results <- applyResult{name: name, err: applyErr}
		}(candidate.name, connection, payload)

		deadline := time.Now().Add(5 * time.Second)
		for {
			select {
			case result := <-results:
				t.Fatalf("concurrent route apply %s bypassed plan lock: %v", result.name, result.err)
			default:
			}
			...
			`, pid).Scan(&waitEventType); err != nil {
				t.Fatalf("inspect concurrent route lock wait for %s: %v", candidate.name, err)
			}
			if waitEventType.Valid && waitEventType.String == "Lock" {
				break
			}
			if !time.Now().Before(deadline) {
				t.Fatalf("concurrent route apply %s did not wait on plan lock", candidate.name)
			}
	...
	if err := blocker.Commit(); err != nil {
		t.Fatalf("release concurrent route blocker: %v", err)
	}
	...
	for range 2 {
		...
		case result := <-results:
			if result.err == nil {
				if success != "" {
					t.Fatalf("both concurrent route sets committed: %s and %s", success, result.name)
				}
				success = result.name
				continue
			}
			...
			if !errors.As(result.err, &postgresError) ||
				(postgresError.ConstraintName != "model_runtime_capacity_route_replay_conflict" &&
					postgresError.ConstraintName != "model_runtime_capacity_route_set_replay_conflict") {
				t.Fatalf("concurrent route apply %s error = %v", result.name, result.err)
			}
		case <-time.After(10 * time.Second):
			t.Fatal("timed out waiting for concurrent route apply")
		}
	}
	if success == "" {
		t.Fatal("neither concurrent ResidencyPlan route set committed")
	}
	if success != "expanded" {
		t.Fatalf("concurrent ResidencyPlan lock queue winner = %s, want expanded", success)
	}
	var routeCount int
	if err := database.Admin.QueryRow(`
		SELECT count(*) FROM model_runtime_capacity_routes
		WHERE residency_plan_revision_id = $1
	`, planID).Scan(&routeCount); err != nil {
		t.Fatalf("count concurrent ResidencyPlan routes: %v", err)
	}
	wantRoutes := 3
	if routeCount != wantRoutes {
		t.Fatalf(
			"concurrent ResidencyPlan route count = %d, want %d for %s winner",
			routeCount, wantRoutes, success,
		)
	}
}
...
func TestWorkerRegistryReleaseRequiresApprovalAndBreakEvenEvidence(t *testing.T) {
	...
	if _, err := stageSchedulerPool.Exec(context.Background(), `
...
func TestWorkerRegistryProposalIsAdvisoryUntilApprovedPlan(t *testing.T) {
	...
	if _, err := stageSchedulerPool.Exec(
		context.Background(),
		"SELECT * FROM vela_apply_residency_plan($1::jsonb)",
		encodedPlan,
...
func approvedResidencyPlanWithSecondRouteFixture(
	proposalID uuid.UUID,
) (map[string]any, string) {
	plan := approvedResidencyPlanFixture(proposalID)
	secondPoolID := "49200000-0000-0000-0000-000000000282"
	secondBundleID := "49200000-0000-0000-0000-000000000283"
	secondWorkerID := uuid.MustParse("49200000-0000-0000-0000-000000000284")
	plan["capacity_pools"] = append(
		plan["capacity_pools"].([]map[string]any),
		map[string]any{
			"id":                        secondPoolID,
			"stable_id":                 "h3-encoder-plan-pool-2",
			"stage_profile_revision_id": "49000000-0000-0000-0000-000000000040",
			"resource_class":            "GPU",
			"security_class":            "INTERNAL",
			"region":                    "cn-shanghai",
	...
	plan["worker_bundles"] = append(
		plan["worker_bundles"].([]map[string]any),
		map[string]any{
			"id":                 secondBundleID,
			"stable_id":          "h3-node-plan-bundle-2",
			"desired_generation": 1,
			"layout_digest":      digestHex(0xd3),
		},
	)
	plan["worker_instances"] = append(
		plan["worker_instances"].([]map[string]any),
		map[string]any{
			"id":                         secondWorkerID.String(),
			"worker_profile_revision_id": workerRegistryProfileID,
			"capacity_pool_id":           secondPoolID,
			"worker_bundle_id":           secondBundleID,
			"desired_member_count":       1,
			"desired_device_count":       1,
			"model_runtime_routes": []map[string]any{{
				"model_residency_id":        uuid.NewSHA1(secondWorkerID, []byte("residency")).String(),
				"capacity_pool_id":          secondPoolID,
				"stage_profile_revision_id": "49000000-0000-0000-0000-000000000040",
			}},
		},
	)
	return plan, secondPoolID
}
